...
{
	size_t len1 = strlen_n(s1), len2 = strlen_n(s2);
...
int AllocateBufferMemory(File_info *tstack)
{
	...
	tstack->strsave = calloc(sizeof(char *), BUFFER_Y);
	tstack->tabcount = malloc(sizeof(int *) * BUFFER_Y);
	tstack->linesize = malloc(sizeof(int *) * BUFFER_Y);
	...
	for (int i = 0; i < tstack->bufy; i++)
	{
		...
		tstack->linesize[i] = calloc(1, sizeof(int));
