...
void DisplayCursorPos(File_info *tstack)
{
	...
	if (fullCursorInfoDisplay)
	{
		...
		printf(longPositionDisplay ? "Line %d/%d (%Lf%%), Column %d/%d (%d%%)" : "Ln %d/%d (%d%%), Col %d/%d (%u%%)", tstack->ypos, tstack->linecount+linecount_zero, 100 * tstack->ypos / (tstack->linecount+linecount_zero), tstack->xpos + 1, linelen + 1, (size_t)100 * (tstack->xpos) / (!linelen ? 1 : linelen));
	}
	else
	{
...
void print_line(char *line, size_t yps)
{
	...
	if (!syntaxHighlighting && Tab_stack[file_index].selection.is_selected)
	{
	...
	{
		color_line(line, Tab_stack[file_index].display_x, 0);
	}
}
